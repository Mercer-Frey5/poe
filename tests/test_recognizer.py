...
class TestDedup:
    def test_url_marks_correlated_domain(self, regex_extractor, fake_spacy):
        ...
        dom = next(e for e in entities if e.type == "domain" and e.value == "evil.com")
        assert dom.metadata.get("derived_source") == "url"

    def test_email_marks_correlated_domain(self, regex_extractor, fake_spacy):
        rec = Recognizer(regex_extractor, fake_spacy([]))
        entities = rec.recognize("Email: user@foo.com e il dominio foo.com")
        assert "user@foo.com" in _values_of(entities, "email")
        dom = next(e for e in entities if e.type == "domain" and e.value == "foo.com")
        assert dom.metadata.get("derived_source") == "email"
...
class TestIntegration:
    def test_full_pipeline(self, regex_extractor, fake_spacy):
        ...
        # v0.5.1: i domini di email/URL ora sono ESTRATTI come correlati (derived)
        assert "paypa1.com" in _values_of(entities, "domain")
        assert "login-paypa1.com" in _values_of(entities, "domain")
