...
from app.scoring import compute_score as _compute_score
...
class TestApiAnalyze:
    """API JSON per consumo programmatico (tool OSINT dell'hub)."""

    def test_returns_entities_json(self, test_client, temp_db):
        r = test_client.post("/api/analyze", json={"text": "scrivimi a mario.rossi@example.com"})
        assert r.status_code == 200
        data = r.json()
        assert data["count"] >= 1
        assert any("mario.rossi@example.com" in e["value"] for e in data["entities"])

    def test_empty_text_rejected(self, test_client, temp_db):
        r = test_client.post("/api/analyze", json={"text": "   "})
        assert r.status_code == 400

    def test_missing_field_422(self, test_client, temp_db):
        r = test_client.post("/api/analyze", json={})
        assert r.status_code == 422       # pydantic: campo 'text' obbligatorio
...
class TestIndexFilter:
    ...
    def test_recent_list_shows_more_than_ten(self, test_client, temp_db):
        """"Osservazioni recenti" era limitato a 10 in ogni route — con più
        di 10 osservazioni le più vecchie sparivano senza modo di vederle."""
        for i in range(15):
            storage.save_observation(f"osservazione numero {i}", [])
        r = test_client.get("/")
        assert r.status_code == 200
        assert r.text.count("history-item") >= 15


class TestOsservazioniHeadingAndLimit:
    """"Osservazioni recenti" -> "Osservazioni" + selettore del numero di
    osservazioni scorrevoli (limit, validato contro un set di valori noti,
    come già fatto per 'view')."""

    def test_heading_renamed(self, test_client, temp_db):
        r = test_client.get("/")
        assert ">Osservazioni<" in r.text
        assert "Osservazioni recenti" not in r.text

    def test_limit_valid_value_applied(self, test_client, temp_db):
        for i in range(15):
            storage.save_observation(f"osservazione numero {i}", [])
        r = test_client.get("/?limit=10")
        assert r.status_code == 200
        assert r.text.count("history-item") == 10

    def test_invalid_limit_falls_back_to_default(self, test_client, temp_db):
        for i in range(35):
            storage.save_observation(f"osservazione numero {i}", [])
        r = test_client.get("/?limit=999999")
        assert r.status_code == 200
        assert r.text.count("history-item") == 30  # _RECENT_LIST_LIMIT

    def test_limit_select_present_with_current_value_selected(self, test_client, temp_db):
        r = test_client.get("/?limit=50")
        assert r.status_code == 200
        assert 'id="history-limit-select"' in r.text
        assert '<option value="50"' in r.text and 'selected' in r.text.split('value="50"')[1][:30]

    def test_history_partial_accepts_limit(self, test_client, temp_db):
        for i in range(15):
            storage.save_observation(f"osservazione numero {i}", [])
        r = test_client.get("/history?view=all&limit=10")
        assert r.status_code == 200
        assert r.text.count("history-item") == 10

    def test_changing_limit_preserves_view_filter(self, test_client, temp_db):
        """Il selettore conteggio non deve resettare il filtro attivo
        (Tutte/Preferiti/Bozze) — il suo hx-get (id=history-limit-select)
        porta con sé ?view= del filtro correntemente attivo."""
        r = test_client.get("/?view=kept&limit=50")
        assert r.status_code == 200
        select_idx = r.text.index('id="history-limit-select"')
        select_tag = r.text[select_idx:select_idx + 200]
        assert 'hx-get="/history?view=kept"' in select_tag


class TestDatabasePage:
    """Pagina 'Database': tutte le osservazioni, non il sottoinsieme
    scorrevole di 'Osservazioni' in home."""

    def test_lists_all_observations_beyond_recent_limit(self, test_client, temp_db):
        for i in range(40):
            storage.save_observation(f"osservazione numero {i}", [])
        r = test_client.get("/database")
        assert r.status_code == 200
        assert r.text.count("history-item") == 40

    def test_respects_view_filter(self, test_client, temp_db):
        oid = storage.save_observation("preferita", [])
        storage.set_kept(oid, True)
        storage.save_observation("bozza", [])
        r = test_client.get("/database?view=kept")
        assert r.status_code == 200
        assert r.text.count("history-item") == 1

    def test_invalid_view_defaults_all(self, test_client, temp_db):
        r = test_client.get("/database?view=nonsense")
        assert r.status_code == 200

    def test_nav_link_present_on_home(self, test_client, temp_db):
        r = test_client.get("/")
        assert 'href="/database"' in r.text

    def test_nav_link_active_on_database_page(self, test_client, temp_db):
        r = test_client.get("/database")
...
class TestSecurityHeaders:
    def test_security_headers_on_index(self, test_client, temp_db):
        ...
        # embedding consentito solo dall'hub vocale in locale (overlay iframe)
        assert "frame-ancestors" in csp and "127.0.0.1:8000" in csp
        assert "x-frame-options" not in resp.headers      # sostituito da frame-ancestors
