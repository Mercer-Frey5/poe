...
def _is_valid_tld(tld: str) -> bool:
    """True se `tld` è una TLD IANA valida: ASCII alfanumerica (len>=2), oppure una
    TLD IDN in forma punycode ('xn--' + label alfanumerica con eventuali trattini).
    Le TLD IDN nel file IANA hanno trattini, quindi `isalnum()` da solo — come faceva
    la validazione originale — le scartava, facendo crashare il refresh (e lasciando
    la lista al seed incompleto: domini con TLD non-seed non riconosciuti)."""
    if len(tld) < 2:
        return False
    if tld.startswith("xn--"):
        rest = tld[4:]
        return bool(rest) and all(c.isalnum() or c == "-" for c in rest) and any(c.isalnum() for c in rest)
    return tld.isalnum()
...
def fetch_iana_tlds() -> tuple[list[str], str]:
    ...
    # Validazione: TLD ASCII alfanumerica (>=2) oppure punycode IDN (xn--...).
    for tld in tlds:
